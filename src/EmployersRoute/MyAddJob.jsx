...
const MyAddJob = () => {
    ...
    return (
        <div className={myAddJobs.length < 6? 'min-h-screen':''}>
            <div className="lg:grid lg:grid-cols-3 gap-5">
                {
                    myAddJobs.map(myAddJob =>
                        <div key={myAddJob._id} className=" border-2 border-gray-200 bg-[#EDE8E0] p-4 rounded-xl my-4  mt-auto">
                            ...
                            <div className="sm:flex justify-between items-center mt-3 ">
                                ...
                                <div className="sm:mr-7">
                                    <p className="flex text-gray-600 items-center text-sm my-1 sm:my-3 gap-3 font-bold sm:mr-3" > <VscGitStashApply /> Candidates : {
                                        myAddJob.applyCandidate > 0 ? <span className="px-2 py-1 bg-[#857569] rounded-full text-xs font-semibold text-white">{myAddJob.applyCandidate}</span> : <span className="px-2 py-1 bg-[#857569] rounded-full text-xs font-semibold text-white">0</span>
                                    }</p>
                            ...
                            <div className="flex justify-between ">
                                <p className="sm:px-8 px-5 mx-auto text-center text-sm my-1 sm:my-1  bg-[#857569] text-white rounded-xl py-1 font-bold hover:bg-[#dad2c5] hover:text-black">
                                    <button onClick={() => handleDelete(myAddJob._id)} className="">Delete</button>
                                </p>
                                <p className="sm:px-8 px-5 mx-auto text-center text-sm my-1 sm:my-1  bg-[#857569] text-white rounded-xl py-1 font-bold hover:bg-[#dad2c5] hover:text-black">
                                    <Link to={`/dashboard/myAddJob/updatePage/${myAddJob._id}`} className="">Update</Link>
                                </p>
                                <p className="sm:px-8 px-5 mx-auto text-center text-sm my-1 sm:my-1  bg-[#857569] text-white rounded-xl py-1 font-bold hover:bg-[#dad2c5] hover:text-black">
                                    <Link to={`/dashboard/myAddJob/candidate/${myAddJob._id}`} className="">View Candidates</Link>
                                </p>
                            </div>
                        </div>)}
            </div>
